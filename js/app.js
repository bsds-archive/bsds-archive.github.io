...
function _updatePageLabel(n) {
  ...
  if (pageCurrentEl) pageCurrentEl.textContent = n;
...
function _initCustomScrollbar() {
  ...
  // reads/writes the DOCUMENT's own scroll position, not .viewer-stage's
  // — see the big comment on .viewer-stage in css/viewer.css for why
  function updateThumb() {
    ...
    const thumbH = Math.max(32, (window.innerHeight / doc.scrollHeight) * trackH);
    const maxThumbTop = trackH - thumbH;
    const thumbTop = Math.min(maxThumbTop, Math.max(0, (window.scrollY / scrollable) * maxThumbTop));
  ...
  function scheduleUpdate() {
    ...
    requestAnimationFrame(() => {
      updateThumb();
      _syncCurrentPageNow(); // single source of truth for "current page" — see that function's comment
      rafPending = false;
    });
  }
  ...
  function jumpToClientY(clientY) {
    ...
    _syncCurrentPageNow(); // don't wait for the IntersectionObserver — see that function's comment
  }
  ...
  // clicking anywhere else on the track jumps straight there — this is
  // a mouse-only convenience. On touch, only the thumb itself responds;
  // the rest of this strip is deliberately left alone so a swipe that
  // happens to start here (e.g. panning a zoomed-in page from near the
  // right edge of the screen) isn't hijacked into a scrollbar jump.
