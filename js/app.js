...
/* ---------------------------------------------------------------------
   ...
   vertical timeline rows on index.html.
--------------------------------------------------------------------- */
...
// Parses the simple "### date | title" + body format described in
// data/announcements.txt. Returns entries sorted newest-first.
function _parseAnnouncements(text) {
  const blocks = text.split(/\n(?=### )/).filter((b) => b.trim().startsWith('###'));
  const entries = blocks.map((block) => {
    const lines = block.trim().split('\n');
    const header = lines[0].replace(/^###\s*/, '');
    const [datePart, ...titleParts] = header.split('|');
    return {
      date: (datePart || '').trim(),
      title: (titleParts.join('|') || 'Untitled').trim(),
      body: lines.slice(1).join('\n').trim(),
      _sortKey: Date.parse((datePart || '').trim()) || 0,
    };
  });
...
function _renderCalendar(calendar) {
  ...
  wrap.innerHTML = Object.entries(groups).map(([month, items]) => `
    <div class="cal-month">
      ...
      ${items.map((item) => `
        <div class="cal-item">
          <span class="cal-date">${_shortDate(item.date)}</span>
...
async function initViewer() {
  ...
  try {
    ...
    _showViewerError(`Couldn't load this PDF at <code>${fileParam}</code>. It may not have been uploaded yet, or the site needs to be served through a local/web server rather than opened directly as a file.`);
    return;
  }
  ...
  // waiting for a page to scroll into view first.
  _pdfPageMeta = [];
...
async function _renderSinglePage(n) {
  ...
  const outputScale = window.devicePixelRatio || 1; // crisp rendering on retina/high-DPI phone screens
...
function _wireViewerControls() {
  ...
  if (searchToggle && searchBar && searchInput) {
    ...
    searchInput.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Escape') { searchInput.value = ''; _runPdfSearch(''); searchBar.classList.remove('open'); }
    });
  }
...
async function _runPdfSearch(query) {
  ...
  _pdfTextIndex.forEach((text, idx) => {
    if (text.includes(q)) _pdfSearchMatchPages.push(idx + 1);
  });
  ...
  if (resultsLabel) resultsLabel.textContent = `${_pdfSearchMatchPages.length} page${_pdfSearchMatchPages.length > 1 ? 's' : ''} matched`;

  // render (or re-render, to pick up highlights) every matching page
  for (const n of _pdfSearchMatchPages) {
    await _renderSinglePage(n);
    _highlightPage(n, query); // covers the case where the page was already rendered before the search started
  }

  _gotoSearchMatch(1); // jumps to the first match
}

// wraps every occurrence of `query` inside a page's (already-rendered)
// text layer spans in a <mark>, so it's visible on top of the canvas
function _highlightPage(n, query) {
  ...
  textLayer.querySelectorAll('span').forEach((span) => {
    ...
    while (true) {
      ...
      if (found === -1) { result += original.slice(idx); break; }
      result += original.slice(idx, found);
      result += `<mark class="pdf-search-mark">${original.slice(found, found + q.length)}</mark>`;
      idx = found + q.length;
    }
...
function _gotoSearchMatch(step) {
  if (!_pdfSearchMatchPages.length) return;
  _pdfSearchMatchIndex = (_pdfSearchMatchIndex + step + _pdfSearchMatchPages.length) % _pdfSearchMatchPages.length;
  const n = _pdfSearchMatchPages[_pdfSearchMatchIndex];

  document.querySelectorAll('.pdf-search-mark.active').forEach((m) => m.classList.remove('active'));
  const wrap = document.querySelector(`.pdf-page-wrap[data-page="${n}"]`);
  if (!wrap) return;
  const firstMark = wrap.querySelector('.pdf-search-mark');
