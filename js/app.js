...
function _syncCurrentPageNow() {
  ...
  const midline = stageTop + stage.clientHeight * 0.4; // roughly where reading focus naturally sits
...
// mid-animation reading, until the scroll settles on its own.
function _scrollToPage(n) {
  ...
  _pdfNavPendingTimer = setTimeout(() => { _pdfNavPending = false; }, 500);
}
